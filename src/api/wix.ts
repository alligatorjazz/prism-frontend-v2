...
/**
 * Bare URL form of the same resolution.
 *
 * The Ricos renderer emits <img> tags and needs a string, not an alt-carrying
 * object, so this is the seam it uses. Keeps every Wix Media lookup in one
 * function.
 */
export function resolveMediaUrl(ref: string | null | undefined): string | null {
  return resolveImageRef(ref, "")?.src ?? null;
}
...
export interface BlogPostDetail {
  ...
  /** Body rendered from the Ricos document, ready for `set:html`. */
  contentHtml: string;
  /** Plain-text body, split into paragraphs. Fallback for contentHtml. */
  paragraphs: string[];
  /** True when the post has a real body, as opposed to none at all. */
  hasContent: boolean;
  /** The raw Ricos document, kept in case a richer renderer is wanted. */
...
export function toBlogPostDetail(
  ...
  // The body. Ricos is the authoritative source; the two text fields are
  // kept as a belt-and-braces fallback because the exact plain-text field
  // name differs between the SDK and the Velo collection (`contentText` vs
  // `plainContent`), and either may be absent. The excerpt is deliberately
  // NOT in this chain — that fallback is what made every post render its
  // own summary as its body.
  const richContent = normalizeRicosDocument(post?.richContent);

  const textBody =
    (post?.contentText ?? post?.plainContent ?? "").trim() ||
    ricosToPlainText(richContent);

  let contentHtml = "";

  try {
    contentHtml = ricosToHtml(richContent, { resolveImage: resolveMediaUrl });
  } catch (error) {
    // A malformed document must not take the whole page down; the plain-text
    // fallback below still renders the post.
    console.error("Could not render rich content for post:", post?.id, error);
  }
  ...
  return {
    ...
    readingTimeMinutes: estimateReadingTime(textBody),
    contentHtml,
    paragraphs: toParagraphs(textBody),
    hasContent: contentHtml.length > 0 || textBody.length > 0,
    richContent: hasRicosContent(richContent) ? richContent : null,
  };
}
